...
def get_emergency_contact(mobile_number):
    conn = sqlite3.connect('app.db')
    cursor = conn.execute(
        "SELECT emergency_contact FROM users WHERE mobile_number = %s" % mobile_number)
    for row in cursor:
...
def get_name_location(mobile_number):
    conn = sqlite3.connect('app.db')
    cursor = conn.execute(
        "SELECT name, address FROM users WHERE mobile_number = %s" % mobile_number)
    result = "Your Friend "
    for row in cursor:
    ...
    cursor = conn.execute(
        "SELECT lattitude, longitude FROM affected_people WHERE mobile_number = %s" % mobile_number)
